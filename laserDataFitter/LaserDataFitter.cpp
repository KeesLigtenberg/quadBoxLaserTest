...
//	std::array<shift,nChips> chipShifts={{ {24.1,15.7}, {38.0, 26.3}, {23.7,26.3}, {9.9,15.7} }};
	...
	struct ChipHistogrammer {
		ChipHistogrammer(const char* name) : dirName(name) {
			...
			nHits=		unique_ptr<TH1I>(new TH1I{"nHits", "Number of hits per event;Number of hits;Entries", 100,0,300});
			constexpr double ToABinWidth=1.5625E-3;
			driftTime=unique_ptr<TH1D>(new TH1D{"driftTime","Drift time;Drift time [#mus];Hits", int(0.2/ToABinWidth),-0.09999,0.1}); //zero would be rounded up (wrong), so move bin slightly to 0.099999
			...
			positionHitMap=unique_ptr<TH2D>(new TH2D{"positionHitMap", "Hitmap with positions;columns;rows",int(30/.055),11,41,int(40/.055),1,41});
  ...
  auto zResidualByToTCorrected=std::unique_ptr<TH2D>(new TH2D{"zResidualByToTCorrected", "z-residual by ToT;ToT [#mus]; z-residual [mm]", 100,0,2.5, 200,-4,6});
	...
	reader.tree->GetEntry();
	 ...
	 do {
		...
		laser=Vec3(50-reader.laser->x, reader.laser->y, 50-reader.laser->z);//mirror z-axis, also mirror x-axis to keep cooridnate system right handed
